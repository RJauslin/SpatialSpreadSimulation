...
#' X_SpatialPoint  <- SpatialPoints(X_SpatialPoint,
#'                               proj4string=CRS("+init=epsg:28992"),
#'                               bbox = NULL)
#'                               
#' # hip.point(x = X_SpatialPoint,n = 75,plot.lattice = TRUE)
#' x = X_SpatialPoint
#' n = 75
#' J = NULL
#' plot.lattice = TRUE
#'         
hip.point2 <- function( x, n, J = NULL, plot.lattice = FALSE ){
  ...
  # for(k in 1:72){ # For each point we are sampling
    ...
    if(plot.lattice){
      text(mean(c(currentBox[1,1],currentBox[1,2])),mean(c(currentBox[2,1],currentBox[2,2])), labels = Sk[k])
      # for(tt in 1:length(pointsInCurrentBox)){
      #   points(pointsInCurrentBox[[tt]][1],
      #          pointsInCurrentBox[[tt]][2],
      #          pch = 16,col = "blue")
      # }
    }
    ...
    while(!pointIsNew){
      ...
      for(i in 1:nrow(samp.coords)){
        if(!all(is.na(samp.coords[i,])) && all(samp.coords[i,] == candidatePoint)){
          pointIsNew <- FALSE
        }
      }
      
      counter <- counter + 1
      if(counter > length(pointsInCurrentBox) & pointIsNew == FALSE){
        stop("The algorithm failed, a box have already all points selected and then you cannot choose a new point.")
      }
